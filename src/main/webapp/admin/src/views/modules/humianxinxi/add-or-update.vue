<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="用户姓名" prop="yonghuxingming">
          <el-select :disabled="ro.yonghuxingming" @change="yonghuxingmingChange" v-model="ruleForm.yonghuxingming" placeholder="请选择用户姓名">
            <el-option
                v-for="(item,index) in yonghuxingmingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
...
export default {
  ...
  methods: {
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              ...
              if (data && data.code === 0) {
                this.yonghuxingmingOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    yonghuxingmingChange () {
      this.$http({
        url: `follow/yonghu/yonghuxingming?columnValue=`+ this.ruleForm.yonghuxingming,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `humianxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.humianxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
